  ...
  return (
    <form className="space-y-4" onSubmit={handleSubmit}>
      <div>
        ...
        <input
          className="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-4 py-2 text-sm focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-200"
          value={username}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6 py-12">
      <div className="relative w-full max-w-md overflow-hidden rounded-2xl border border-slate-100 bg-white p-8 shadow-card">
        <div className="pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-red-500 via-orange-400 to-amber-400" />
        <div className="pointer-events-none absolute -right-12 -top-12 h-32 w-32 rounded-full bg-red-100/70 blur-2xl" />
        <div className="mb-8">
          <div className="mb-5 flex items-center gap-4">
            <div className="rounded-full bg-white p-1 shadow-md ring-4 ring-red-100">
              <img
                src={logo}
        ...
        <div className="mb-6 flex rounded-lg border border-slate-200 bg-slate-50 p-1">
          <button
            type="button"
            className={`flex-1 rounded-md px-3 py-2 text-xs font-semibold uppercase tracking-[0.2em] ${
              activeTab === "admin"
        ...
        <div className="mt-6 rounded-xl border border-slate-200 p-4">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-5">
            <div>
              ...
              <input
                type="date"
                className="mt-2 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-200"
            ...
            <div>
              <label className="text-xs font-medium text-slate-500">Doacao</label>
              <input
                type="number"
                min="0"
                step="0.01"
...
function UserDashboard({ token, onLogout }) {
  ...
  return (
    <div className="min-h-screen">
      <header className="relative overflow-hidden bg-red-600 text-white">
        <div className="pointer-events-none absolute -left-20 top-0 h-40 w-40 rounded-full bg-white/20 blur-2xl" />
        <div className="pointer-events-none absolute -right-24 -top-10 h-44 w-44 rounded-full bg-orange-300/30 blur-2xl" />
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-xs uppercase tracking-[0.2em] text-red-100/90">
                Gestao 2026-2027
              </p>
            </div>
          </div>
                    ...
                    {loading ? (
                      ...
                      payments.map((item) => (
                        <tr key={item.idmensalidade} className="text-slate-700">
                          <td className="py-3 pr-4">{item.competencia}</td>
                          ...
                          <td className="py-3 pr-4">{item.data_pagamento}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
...
function Dashboard({ token, onLogout }) {
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(async () => {
      ...
      setError("");
      try {
        const query = filteredSearch ? `?search=${encodeURIComponent(filteredSearch)}` : "";
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10">
        <div className="relative flex flex-col gap-4 overflow-hidden rounded-2xl bg-white p-6 shadow-card">
          <div className="pointer-events-none absolute inset-x-0 -top-16 h-32 bg-gradient-to-r from-red-100/80 via-orange-50 to-amber-100/60" />
          <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <h2 className="text-xl font-display text-slate-900">Associados</h2>
              <p className="text-sm text-slate-500">
                Lista principal de inscritos cadastrados.
              </p>
            ...
            <div className="relative w-full md:w-80">
              ...
              <input
                className="w-full rounded-lg border border-slate-200 bg-slate-50 py-2 pl-9 pr-3 text-sm focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-200"
